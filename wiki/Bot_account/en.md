...
A **bot account** is a special type of osu! account run by an automated service instead of a real person. On the website, they have a white [group badge](/wiki/Community/Group#badge) labelled `BOT`, and only the `me!` section of their profile is visible. Bot accounts are most commonly used to host "chat bots" that offer various functionalities to the community through [multiplayer](/wiki/Gameplay/Multiplayer) lobby channels or private messages.
...
- The `BOT` group badge is shown on accounts with a [primary group](/wiki/Community/Group#primary-group) of `Chat Bots` (group #29). The group's listing is not public.
